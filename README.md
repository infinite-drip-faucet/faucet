...
A small Monero (RandomX) mining pool that proxies its combined hashrate to an
upstream pool, credits miners with an ERC-20 token on an L2, and lets them
redeem that token for actual XMR. The entire pool runs as a single binary
...
- Watches `Redemption` events on the L2. When a miner burns MPT, a
